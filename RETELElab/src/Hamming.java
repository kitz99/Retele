...
class Hamming {
	public static void main(String arg[]) {
		...
		System.out.println("Introduceti info (7 biti)");
		int d[] = new int[7];
		for (int i = 0; i < 7; i++) {
			d[i] = sc.nextInt();
		}
		...
		int c[] = new int[11];
		System.out.println("Informatia codificata si transmisa: ");
		...
		System.out.println();
		System.out.println("Informatia receptionata: ");
		int r[] = new int[11];
		for (int i = 0; i < 11; i++) {
			r[i] = sc.nextInt();
		}
		...
		if (dec == 0)
			...
			System.out.println("Eroarea la pozitia " + dec);
			if (r[dec - 1] == 0)
				r[dec - 1] = 1;
			else
				r[dec - 1] = 0;
		}
		System.out.println("Codul corectat : ");
		for (int i = 0; i < 11; i++)
